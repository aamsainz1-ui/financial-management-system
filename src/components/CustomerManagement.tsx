...
export default function CustomerManagement() {
  ...
  const handleDelete = async (id: string) => {
    ...
    toast({
      ...
      description: `ลบลูกค้า "${customerToDelete.name}" เรียบร้อยแล้ว`,
    });
    ...
    try {
      ...
      toast({
        ...
        description: "ไม่สามารถลบลูกค้าได้ กำลังคืนข้อมูล",
        variant: "destructive",
      });
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Dialog open={isTransactionDialogOpen} onOpenChange={setIsTransactionDialogOpen}>
            ...
            <DialogContent className="max-w-md">
              ...
              <form onSubmit={handleTransactionSubmit} className="space-y-4">
                {transactionData.type !== 'extension' && (
                  <div>
                ...
                <div>
                  <Label htmlFor="amount">
                    {transactionData.type === 'extension' ? 'จำนวนเงินต่อยอด' : 'จำนวนเงิน'}
                  </Label>
                  <Input
                    id="amount"
                    type="number"
                ...
                {transactionData.type === 'extension' && (
                  <div>
                    <Label htmlFor="extensionCustomer">ลูกค้าที่ต่อยอด</Label>
                    <Select
                      value={transactionData.customerId}
                      onValueChange={(value) => setTransactionData(prev => ({ ...prev, customerId: value }))}
                    >
                      <SelectTrigger>
                        <SelectValue placeholder="เลือกลูกค้าที่ต่อยอด" />
                      </SelectTrigger>
                      <SelectContent>
                        {customers.map((customer) => (
                          <SelectItem key={customer.id} value={customer.id}>
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    placeholder={transactionData.type === 'extension' ? 'รายละเอียดการต่อยอด...' : 'รายละเอียดอื่นๆ...'}
                  />
                </div>
          ...
          <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
            ...
            <DialogContent className="max-w-md mx-auto max-h-[90vh] overflow-y-auto sm:max-w-2xl">
              <DialogHeader>
                <DialogTitle className="text-lg">{editingCustomer ? 'แก้ไขข้อมูลลูกค้า' : 'เพิ่มลูกค้าใหม่'}</DialogTitle>
                <DialogDescription>
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem value="">ไม่ระบุทีม</SelectItem>
                        {teams.map((team) => (
                    ...
                    {formData.teamId && teams.find(t => t.id === formData.teamId) && (
                      <p className="text-sm text-gray-500">
                        ทีมที่เลือก: {teams.find(t => t.id === formData.teamId)?.name}
                      </p>
                    )}
                  </div>
                  <div>
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem value="">ไม่ระบุพนักงาน</SelectItem>
                        {members.map((member) => (
                    ...
                    {formData.memberId && members.find(m => m.id === formData.memberId) && (
                      <p className="text-sm text-gray-500">
                        พนักงานที่เลือก: {members.find(m => m.id === formData.memberId)?.name}
                      </p>
                    )}
                  </div>
                </div>
                ...
                <TableBody>
                  {customers.map((customer) => (
                    <TableRow key={customer.id}>
                      ...
                      <TableCell>฿{(customer.initialAmount || 0).toLocaleString()}</TableCell>
                      <TableCell>
                        <div className="flex gap-2">
                          <Button
                            variant="outline"
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  คุณต้องการลบลูกค้า "{customer.name}" ใช่หรือไม่?
