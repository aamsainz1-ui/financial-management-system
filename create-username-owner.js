...
console.log('🔧 สร้าง Owner ด้วย username ที่ถูกต้อง...\n');
...
bcrypt.hash(newPassword, 12).then(async (hashedPassword) => {
  ...
  if (fs.existsSync(prismaDbPath)) {
    try {
      ...
      if (tableExists) {
        ...
        // สร้าง Owner ใหม่ด้วย username และ bcrypt hash
  ...
  console.log('\n🎯 ข้อมูล Login ใหม่ (ที่ถูกต้อง):');
  console.log('   👤 Username: owner');
  console.log('   🔑 Password: ' + newPassword);
  console.log('\n⚠️  ใช้ username "owner" ไม่ใช่ email!');
